...
//    }
    ...
    void findAllMissionsTest() {
        ...
        // 참여하고 있는 스터디 아이디
        List<Study> studyIds = Arrays.asList(
                studyRepository.findById(1L).orElseThrow(() -> new NotFoundStudyException(ErrorCode.STUDY_NOT_EXIST))
                ,
                studyRepository.findById(2L).orElseThrow(() -> new NotFoundStudyException(ErrorCode.STUDY_NOT_EXIST)));
        System.out.println("\n>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>");
